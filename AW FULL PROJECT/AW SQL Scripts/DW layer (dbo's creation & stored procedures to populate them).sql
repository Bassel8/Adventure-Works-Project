...
SELECT stg.[ProductNumber]
      ...
      ,stg.[ProductCategory]   
	  ,null [Status]
  FROM [AW_STG].[dbo].[Stg_vw_Erp_Product] stg (nolock)
  ...
  inner join [AW_STG].[dbo].[Stg_vw_Erp_Product] stg (nolock)
  on Dim.[ProductAlternateKey] = stg.[ProductNumber]
...
,[SalesTerritoryGroup]=[Group]
  ...
  inner join [AW_STG].[erp].[SalesTerritory] stg  (nolock)
  on Dim.[SalesTerritoryAlternateKey] = stg.[TerritoryID]
...
SELECT  Stg.[StoreID]
	  ...
	  ,Stg.[NumberEmployees] 
	  ,Stg.[YearOpened] 
  FROM [AW_STG].[dbo].[Stg_vw_Erp_Reseller] stg (nolock)
...
, Dim.[NumberEmployees]=stg.[NumberEmployees]
  ...
  inner join  [AW_STG].[dbo].[Stg_vw_Erp_Reseller]  stg  (nolock)
  on  stg.[StoreID] = Dim.[ResellerAlternateKey]
